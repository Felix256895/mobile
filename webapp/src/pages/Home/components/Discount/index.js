    ...
    render() {
        return (
            <div className={styles.discount}>
                <Link to='/deadlist' className={styles.discount__header}>
                    <span className={styles.discount__title}>超值特惠</span>
                    <span className={styles.discount__more}>更多优惠</span>
                    <span className={styles.discount__arrow} />
                </Link>
                <div className={styles.discount__content}>
                    {
                        dataSource.map(item=>{
                            return (
                                <Link key={item.id} to='/detail' className={styles.discount__item}>
                                    <div className={styles.discount__itemPic}>
                                        <img src={item.picture} alt=""/>
                                    </div>
                                    <div className={styles.discount__itemTitle}>{item.shop}</div>
                                    <div className={styles.discount__priceWrapper}>
                                        <ins className={styles.discount__itemCurrentPrice}>{item.currentPrice}</ins>
                                        <del className={styles.discount__itemOldPrice}>{item.oldPrice}</del>
                                    </div>
                                </Link>
                            )
                        })
